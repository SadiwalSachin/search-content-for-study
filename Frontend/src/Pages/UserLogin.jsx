...
import { setUserData, setIsLoggedIn } from "../redux/Slices/userSlice.js";
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://search-content-user-service.onrender.com/api/v1/user/login",
        userInfo
      );
        ...
        <div className="w-[90%] md:w-[35%] rounded-xl border-2 border-black py-2 flex flex-col gap-y-3">
          <form onSubmit={onSubmitHandler} action="" className="w-full h-full">
            ...
            <div className="w-full px-5 flex flex-col gap-y-2 mt-2">
              ...
              <button
                disabled={loading}
                className={`mt-4 bg-[#18181B] text-white py-2  rounded-md ${
                  loading ? "opacity-50" : ""
                }`}
              >
                Login
